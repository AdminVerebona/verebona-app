...
export async function POST(
  ...
  try {
    ...
    if (duo.subscriptionStatus === 'UNPAID_RECOVERY') {
      return NextResponse.json(
        { error: 'FORBIDDEN_IN_RECOVERY', message: 'Les demandes de suppression sont interdites en mode recovery' },
        { status: 403 }
      );
    }
    ...
    await DuoService.createDeleteRequest({
      assetId,
      duoId: duo.id,
    ...
    return NextResponse.json({ success: true, message: 'Demande de suppression créée' });
  } catch (error: any) {
    console.error('[ASSET_DELETE_REQUEST] Error:', error);

    if (error.message === 'PENDING_REQUEST_EXISTS') {
      return NextResponse.json(
        { error: 'PENDING_REQUEST_EXISTS', message: 'Une demande de suppression est déjà en cours pour ce bien' },
        { status: 409 }
      );
    }

    if (error.message === 'PENDING_MOVE_REQUEST_EXISTS') {
      return NextResponse.json(
        { error: 'PENDING_MOVE_REQUEST_EXISTS', message: 'Une demande de déplacement est en cours pour ce bien' },
        { status: 409 }
      );
    }
