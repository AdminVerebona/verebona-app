 ...
 *
 * Prévisualise le montant de proration Stripe pour un passage Premium → Premium Duo.
 ...
 * Réponse:
 * {
 *   current_offer: "premium",
 *   target_offer: "premium_duo",
 *   billing_cycle_anchor_kept: true,
 *   next_renewal_date: "2027-03-15",
 *   proration_amount_due_now: 10.00,
 *   currency: "eur"
 * }
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Prévisualisation de la proration via l'API Stripe
    ...
    // Montant dû immédiatement (en centimes → euros)
    const amountDueNow = Math.max(0, upcomingInvoice.amount_due) / 100;
    ...
    return NextResponse.json({
      ...
      proration_amount_due_now: amountDueNow,
