...
ALTER TABLE event_documents
  ADD CONSTRAINT event_documents_event_file_unique UNIQUE (event_id, file_id);
...
ALTER TABLE dunning_events
  ADD CONSTRAINT dunning_events_duo_stage_unique UNIQUE (duo_id, stage);
...
CREATE UNIQUE INDEX account_memberships_unique_active_idx
...
ALTER TABLE assets
  ADD CONSTRAINT assets_copy_source_request_id_fkey
...
ALTER TABLE document_versions
  ADD CONSTRAINT document_versions_file_version_unique UNIQUE (file_id, version_number);

-- ─── 8. duo_memberships: unique (duo_id, user_id) ────────────────────────────
ALTER TABLE duo_memberships
  ADD CONSTRAINT duo_memberships_duo_user_unique UNIQUE (duo_id, user_id);

-- ─── 9. asset_custom_field_values: unique (asset_id, field_id) ───────────────
ALTER TABLE asset_custom_field_values
  ADD CONSTRAINT asset_custom_field_values_asset_field_unique UNIQUE (asset_id, field_id);
...
ALTER TABLE document_type_asset_associations
  ADD CONSTRAINT doc_type_asset_assoc_unique
...
ALTER TABLE document_type_export_associations
  ADD CONSTRAINT doc_type_export_assoc_unique
...
ALTER TABLE assets
  ADD CONSTRAINT assets_lock_state_check
...
ALTER TABLE duo_memberships
  ADD CONSTRAINT duo_memberships_status_check
