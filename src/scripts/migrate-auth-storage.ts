 ...
 *
 * Usage :
 *   npx tsx scripts/migrate-auth-storage.ts --dry-run   # simulation
 *   npx tsx scripts/migrate-auth-storage.ts             # application
 *
...
function migrate(source: string): { output: string; report: Omit<Report, 'file'> } {
  ...
  // 2. Gardes simples : if (!token) return; / return null; / return false;
  out = out.replace(
    new RegExp(`^[ \\t]*if \\(!(?:${vars})\\) \\{?\\s*return(?: [^;]*)?;\\s*\\}?[ \\t]*\\r?\\n`, 'gm'),
  ...
  out = out.replace(
    /fetch\((['"`])(\/api\/[^'"`]*)\1,\s*\{\s*\r?\n/g,
    (match) => {
      if (/credentials/.test(match)) return match;
      addedCredentials++;
  ...
  if (new RegExp(`if \\(!(?:${vars})\\)`).test(out)) {
...
function main() {
  ...
  for (const file of files) {
    ...
    if (!/bearer_token|Bearer \$\{|credentials: 'include'/.test(source)) continue;
